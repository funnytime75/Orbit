...
暂不在 M0 实现 Windows 全局鼠标 hook、动作执行器和配置落盘。
...
| Command | 职责 | M0 状态 |
| --- | --- | --- |
| `load_config` | 读取当前配置 | 已实现默认配置 |
| `validate_config` | 校验配置 | 已实现 |
| `get_runtime_status` | 返回运行状态 | 已实现 |
| `save_config` | 校验并保存配置 | 后续实现 |
| `execute_action` | 根据动作 ID 执行动作 | 后续实现 |
| `set_orbit_enabled` | 启用或停用全局监听 | 后续实现 |
| `open_settings_window` | 打开设置窗口 | 后续实现 |
