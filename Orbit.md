...
- Windows `.exe` 应用执行器。
- Windows 鼠标中键长按触发、方向选择、松开执行和轮盘隐藏。
- 全局组合键辅助触发配置。
- 开机自启、静默启动和托盘入口。

暂未实现非应用动作执行、多显示器工作区精确夹紧和真实背景图片渲染。

## 2. 项目定位

Orbit 通过鼠标中键长按在光标位置打开轮盘菜单，并通过拖动方向和松开执行快速启动应用。键盘组合键可作为辅助入口打开轮盘，但首要交互仍是中键长按手势。
...
- `trigger.shortcut` 必须是键盘组合键，至少包含 `Ctrl`、`Alt`、`Shift` 或 `Win` 中一个修饰键和一个主按键。
...
- `MiddleDown` 后记录按下位置和时间。
- 持续按下超过 `holdMs` 且移动超过 `moveThresholdPx` 才显示轮盘。
- 轮盘窗口以按下位置为中心弹出，而不是以显示时的鼠标位置重置原点。
- 松开中键后按按下位置到当前鼠标位置的方向计算扇区。
- 松开时仍在取消距离或中心死区内则取消，否则执行选中动作并隐藏轮盘。
- hook 回调只做轻量状态更新，窗口控制和动作执行转交后台 worker。

## 12.1 键盘组合键辅助触发策略

要求：

- 全局快捷键注册使用 `trigger.shortcut`。
- 触发快捷键必须是组合键，禁止单键触发。
- 快捷键触发后显示轮盘，作为辅助入口保留。
...
7. 关闭设置窗口后可从托盘重新打开。
8. 长按鼠标中键、拖向某个扇区并松开后，选中应用会启动且轮盘隐藏。
...
状态：已接入 Windows 中键长按触发、轮盘窗口定位、移动高亮、松开执行和隐藏；多显示器工作区精确夹紧后续完善。
